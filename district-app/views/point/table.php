		...
		<form id="mainform" name="mainform" action="" method="post">
          ...
          <?php $this->load->view('plan/nav.php')?>
            ...
            <div class="card-header">
							<a href="<?=site_url("point/form")?>" class="btn btn-success btn-sm"  title="Tambah Jenis Lokasi Baru">
								<i class="fe fe-plus"></i>Tambah Jenis Lokasi Baru
            	</a>
							<?php if ($this->CI->cek_hak_akses('h')): ?>
								<a href="#confirm-delete" title="Hapus Data" onclick="deleteAllBox('mainform', '<?=site_url("point/delete_all/$p/$o")?>')" class="btn btn-outline-danger btn-sm btn-sm hapus-terpilih"><i class='fe fe-trash'></i> Hapus Data Terpilih</a>
							<?php endif; ?>
													...
													<div class="box-tools">
														<div class="input-group input-group-sm pull-right">
															...
															<div class="input-group-btn">
																<button type="submit" class="btn btn-default" onclick="$('#'+'mainform').attr('action', '<?=site_url("point/search")?>');$('#'+'mainform').submit();"><i class="fe fe-search"></i></button>
															</div>
														</div>
													</div>
												</div>
														...
														<table class="table table-bordered table-striped dataTable table-hover">
															<thead class="bg-gray disabled color-palette">
																<tr>
																	...
																	<th>Aksi</th>
																	<?php if ($o==2): ?>
                                    <th><a href="<?= site_url("point/index/$p/1")?>">Jenis <i class='fe fe-sort-asc fa-sm'></i></a></th>
                                  <?php elseif ($o==1): ?>
                                    <th><a href="<?= site_url("point/index/$p/2")?>">Jenis <i class='fe fe-sort-desc fa-sm'></i></a></th>
                                  <?php else: ?>
                                    <th><a href="<?= site_url("point/index/$p/1")?>">Jenis <i class='fe fe-sort fa-sm'></i></a></th>
                                  <?php endif; ?>
                                  <?php if ($o==4): ?>
                                    <th nowrap><a href="<?= site_url("point/index/$p/3")?>">Aktif <i class='fe fe-sort-asc fa-sm'></i></a></th>
                                  <?php elseif ($o==3): ?>
                                    <th nowrap><a href="<?= site_url("point/index/$p/4")?>">Aktif <i class='fe fe-sort-desc fa-sm'></i></a></th>
                                  <?php else: ?>
                                    <th nowrap><a href="<?= site_url("point/index/$p/3")?>">Aktif <i class='fe fe-sort fa-sm'></i></a></th>
                                  <?php endif; ?>
																	<th>Simbol</th>
																</tr>
															</thead>
																	...
																	<tr>
																		...
																		<td nowrap>
																			...
																			<?php elseif ($data['enabled'] == '1'): ?>
																				<a href="<?= site_url('point/point_unlock/'.$data['id'])?>" class="btn bg-navy btn-box btn-sm" title="Non Aktifkan"><i class="fe fe-unlock"></i></a>
																			<?php endif; ?>
																			<a href="#" data-href="<?= site_url("point/delete/$p/$o/$data[id]")?>" class="btn btn-outline-info btn-sm"  title="Hapus" data-toggle="modal" data-target="#confirm-delete"><i class="fe fe-trash"></i></a>
																	  </td>
