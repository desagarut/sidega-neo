						...
						<form class="form-inline">
							<div class="form-group mb-2 ">
								<?php if ($this->CI->cek_hak_akses('h')) : ?>
									<a href="<?= site_url('penduduk/form'); ?>" class="btn btn-primary mr-1" title="Tambah Data"><i class="fe fe-plus"></i> Penduduk Domisili</a>
									<a href="#confirm-delete" title="Hapus Data Terpilih" onclick="deleteAllBox('mainform', '<?= site_url("penduduk/delete_all/$p/$o"); ?>')" class="btn btn-outline-danger mr-1 hapus-terpilih"><i class='fe fe-trash-o'></i> Hapus Data Terpilih</a>
								<?php endif; ?>

								<button class="btn btn-outline-primary dropdown-toggle mr-1" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Aksi Lainnya <span class="text-muted sr-only">Action</span></button>
								...
								<a href="<?= site_url("{$this->controller}/clear"); ?>" class="btn btn-outline-primary mr-1"><i class="fe fe-refresh"></i>Bersihkan</a>
							</div>
						</form>
					</div>
				</div>
											...
											<?php if ($main) : ?>
												<?php foreach ($main as $key => $data) : ?>
													<tr>
														...
														<td>
															<?= strtoupper($data['alamat']); ?>, RT <?= strtoupper($data['rt']); ?> / RW <?= strtoupper($data['rw']); ?> Dusun <?= strtoupper($data['dusun']); ?>
														</td>
