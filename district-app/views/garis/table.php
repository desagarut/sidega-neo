          ...
          <?php $this->load->view('plan/nav.php')?>
            ...
            <div class="box-header with-border">
							...
							<?php if ($this->CI->cek_hak_akses('h')): ?>
								<a href="#confirm-delete" title="Hapus Data" onclick="deleteAllBox('mainform', '<?= site_url("garis/delete_all/$p/$o")?>')" class="btn btn-social btn-box btn-danger btn-sm visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block hapus-terpilih"><i class='fe fe-trash-o'></i> Hapus Data Terpilih</a>
							<?php endif; ?>
													...
													<div class="box-tools">
														<div class="input-group input-group-sm pull-right">
															...
															<div class="input-group-btn">
																<button type="submit" class="btn btn-default" onclick="$('#'+'mainform').attr('action', '<?= site_url("garis/search")?>');$('#'+'mainform').submit();"><i class="fe fe-search"></i></button>
															</div>
														</div>
													</div>
												</div>
														...
														<table class="table table-bordered table-striped dataTable table-hover">
															<thead class="bg-gray disabled color-palette">
																<tr>
																	...
																	<th>Aksi</th>
																	<?php if ($o==2): ?>
                                    <th><a href="<?= site_url("garis/index/$p/1")?>">Garis <i class='fe fe-sort-asc fa-sm'></i></a></th>
                                  <?php elseif ($o==1): ?>
                                    <th><a href="<?= site_url("garis/index/$p/2")?>">Garis <i class='fe fe-sort-desc fa-sm'></i></a></th>
                                  <?php else: ?>
                                    <th><a href="<?= site_url("garis/index/$p/1")?>">Garis <i class='fe fe-sort fa-sm'></i></a></th>
                                  <?php endif; ?>
                                  <?php if ($o==4): ?>
                                    <th nowrap><a href="<?= site_url("garis/index/$p/3")?>">Aktif <i class='fe fe-sort-asc fa-sm'></i></a></th>
                                  <?php elseif ($o==3): ?>
                                    <th nowrap><a href="<?= site_url("garis/index/$p/4")?>">Aktif <i class='fe fe-sort-desc fa-sm'></i></a></th>
                                  <?php else: ?>
                                    <th nowrap><a href="<?= site_url("garis/index/$p/3")?>">Aktif <i class='fe fe-sort fa-sm'></i></a></th>
                                  <?php endif; ?>
																	<th>Kategori</th>
																	<th>Jenis</th>
																</tr>
															...
															<tbody>
																<?php foreach ($main as $data): ?>
																	<tr>
																		...
																		<td><?=$data['no']?></td>
																		<td nowrap>
																			<a href="<?= site_url("garis/form/$p/$o/$data[id]")?>" class="btn btn-warning btn-box btn-sm"  title="Ubah"><i class="fe fe-edit"></i></a>
																			...
																			<?php if ($data['enabled'] == '2'): ?>
																				<a href="<?= site_url('garis/garis_lock/'.$data['id'])?>" class="btn bg-navy btn-box btn-sm" title="Aktifkan"><i class="fe fe-lock">&nbsp;</i></a>
