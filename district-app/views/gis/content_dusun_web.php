	...
	<?php foreach ($dusun_gis as $key_dusun => $dusun): ?>
		<div id="isi_popup_dusun_<?= $key_dusun ?>" style="visibility: hidden;">
			<div id="content">
				...
				<div id="bodyContent">
					<p><center><a href="#collapseStatGraph" class="btn btn-outline-info btn-sm" title="Statistik Penduduk" data-toggle="collapse" data-target="#collapseStatGraph" aria-expanded="false" aria-controls="collapseStatGraph"><i class="fe fe-bar-chart"></i>&nbsp;&nbsp;Statistik Penduduk&nbsp;&nbsp;</a></center></p>
					...
					<p><center><a href="<?=site_url("load_aparatur_wilayah/$dusun[id_kepala]/1")?>" class="btn btn-outline-info btn-sm" data-title="Kepala <?= set_ucwords($wilayah) . $dusun['dusun']?>" data-remote="false" data-toggle="modal" data-target="#modalKecil"><i class="fe fe-user"></i>&nbsp;Kepala <?= set_ucwords($wilayah)?>&nbsp;</a></center></p>
				</div>
			</div>
		</div>
	<?php endforeach; ?>
