				...
				for (var x = 0; x < jml; x++) {
					if (penduduk[x].lat || penduduk[x].lng) {
						...
						content =
							...
							'<p><a href="<?= site_url("penduduk/detail/1/0/") ?>' + penduduk[x].id + '" class="btn btn-info btn-sm" target="ajax-modalx" rel="content" header="Rincian Data ' + penduduk[x].nama + '" >Lihat Detail</a></p></td>' +
							'</tr></table>';
              ...
              <div class="leaflet-top leaflet-right">
                ...
                <div id="target1" class="leaflet-control-layers leaflet-control-layers-expanded leaflet-control hidden" aria-haspopup="true" style="max-width: 250px;">
                  <div class="leaflet-control-layers-overlays">
                    <div class="leaflet-control-layers-group" id="leaflet-control-layers-group-2"> <span class="leaflet-control-layers-group-name">CARI PENDUDUK</span>
                      <div class="leaflet-control-layers-separator"></div>
                      <div class="form-group">
                        <label>Status Penduduk</label>
                        <select class="form-control select2 input-sm " name="filter" onchange="formAction('mainform_map','<?= site_url('gis/filter') ?>')">
                      ...
                      <div class="form-group">
                        ...
                        <select class="form-control select2 input-sm " name="sex" onchange="formAction('mainform_map','<?= site_url('gis/sex') ?>')">
                      ...
                      <div class="form-group">
                        ...
                        <select class="form-control select2 input-sm " name="dusun" onchange="formAction('mainform_map','<?= site_url('gis/dusun') ?>')">
                          <option value="">Pilih Dusun</option>
                          <?php foreach ($list_dusun as $data) : ?>
                          <option value="<?= $data['dusun'] ?>" <?= selected($dusun, $data['dusun']); ?>>
                          <?= $data['dusun'] ?>
                          </option>
                          <?php endforeach; ?>
                        </select>
                      ...
                      <div class="form-group">
                        <label>RW</label>
                        <select class="form-control select2 input-sm " name="rw" onchange="formAction('mainform_map','<?= site_url('gis/rw') ?>')">
                      ...
                      <div class="form-group">
                        <label>RT</label>
                        <select class="form-control select2 input-sm " name="rt" onchange="formAction('mainform_map','<?= site_url('gis/rt') ?>')">
                          <option value="">Pilih RT</option>
                          <?php foreach ($list_rt as $data) : ?>
                          <option value="<?= $data['rt'] ?>" <?= selected($rt, $data['rt']); ?>>
                          <?= $data['rt'] ?>
                          </option>
                          <?php endforeach; ?>
                        </select>
                      ...
                      <div class="form-group"> <a href="<?= site_url("gis/ajax_adv_search") ?>" class="btn btn-outline-info btn-sm" data-remote="false" data-toggle="modal" data-target="#modalBox" data-title="Pencarian Spesifik" title="Pencarian Spesifik"> <i class="fe fe-search"></i> Pencarian Spesifik </a> <a href="<?= site_url("gis/clear") ?>" class="btn btn-outline-info btn-sm"> <i class="fe fe-refresh"></i> Bersihkan </a> </div>
                    </div>
                  </div>
                </div>
                <div id="target2" class="leaflet-control-layers leaflet-control-layers-expanded leaflet-control hidden" aria-haspopup="true" style="max-height: 315px;">
                  <div class="leaflet-control-layers-overlays">
                    <div class="leaflet-control-layers-group" id="leaflet-control-layers-group-3"> <span class="leaflet-control-layers-group-name">LOKASI</span>
                      <div class="leaflet-control-layers-separator"></div>
                      <label>
